...
class TestReview_instantiation(unittest.TestCase):
    ...
    def place_id_is_public_class_attribute_test(self):
        rv = Review()
    ...
    def text_is_public_class_attribute_test(self):
        rv = Review()
        self.assertEqual(str, type(Review.text))
        self.assertIn("text", dir(rv))
        self.assertNotIn("text", rv.__dict__)

    def two_reviews_unique_ids_test(self):
        rv1 = Review()
        rv2 = Review()
        self.assertNotEqual(rv1.id, rv2.id)

    def two_reviews_different_created_at_test(self):
        rv1 = Review()
        sleep(0.05)
        rv2 = Review()
        self.assertLess(rv1.created_at, rv2.created_at)

    def two_reviews_different_updated_at_test(self):
        rv1 = Review()
        sleep(0.05)
        rv2 = Review()
